...
impl TodoService {
    ...
    pub fn create_todo(&mut self, create_todo_dto: CreateTodoDTO) -> Result<Todo, Error> {
    ...
    pub fn get_all_todos(&mut self) -> Result<Vec<Todo>, Error> {
        let todos = get_todos();
        Ok(todos)
    }

    pub fn delete_todo(&mut self, todo_id: i32) -> Result<(), Error> {
        delete_todo(todo_id)
    }
}
